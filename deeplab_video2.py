...
def apply_mask(image, mask):
    time_mask0 =time.time()
    image = np.array(image)
    print('image_shape {}'.format(image.shape))
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    mask = label_to_color_image(mask).astype(np.uint8)
    ...
    time_mask1 = time.time()
    print('time_mask {}'.format(time_mask1-time_mask0))
    return image
